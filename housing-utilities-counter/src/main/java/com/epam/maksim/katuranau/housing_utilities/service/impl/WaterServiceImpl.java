...
public class WaterServiceImpl implements WaterService {
    ...
    private static final Double LEFT_WATER_SERVICE_RESOURCES_AMOUNT_LIMIT = 0.5;
    private static final Double RIGHT_WATER_SERVICE_RESOURCES_AMOUNT_LIMIT = 1.5;

    private CustomSource source;

    @Autowired
    public WaterServiceImpl(final CustomSource source) {
        this.source = source;
    }
...
//        }
    ...
    private BigDecimal generateAmountOfResourcesSpent() {
        return BigDecimal.valueOf(LEFT_WATER_SERVICE_RESOURCES_AMOUNT_LIMIT + Math.random()
                * (RIGHT_WATER_SERVICE_RESOURCES_AMOUNT_LIMIT - LEFT_WATER_SERVICE_RESOURCES_AMOUNT_LIMIT));
    }

    private void sendMessage(final Message message) {
        source.waterServiceOutput().send(MessageBuilder
                .withPayload(message)
