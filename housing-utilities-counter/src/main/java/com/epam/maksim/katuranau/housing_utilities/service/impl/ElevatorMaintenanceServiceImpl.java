...
public class ElevatorMaintenanceServiceImpl implements ElevatorMaintenanceService {
    ...
    private static final Double LEFT_LIMIT_OF_NUMBER_OF_PEOPLE_LIVING = 1.0;
    private static final Double RIGHT_LIMIT_OF_NUMBER_OF_PEOPLE_LIVING = 7.0;

    private CustomSource source;

    @Autowired
    public ElevatorMaintenanceServiceImpl(final CustomSource source) {
        this.source = source;
    }
    ...
    private BigDecimal generateAmountOfResourcesSpent() {
        return BigDecimal.valueOf(LEFT_LIMIT_OF_NUMBER_OF_PEOPLE_LIVING + Math.random()
                * (RIGHT_LIMIT_OF_NUMBER_OF_PEOPLE_LIVING - LEFT_LIMIT_OF_NUMBER_OF_PEOPLE_LIVING));
    }

    private void sendMessage(final Message message) {
        source.elevatorMaintenanceOutput().send(MessageBuilder
                .withPayload(message)
