...
@Service
public class ElevatorMaintenanceServiceImpl implements ElevatorMaintenanceService {
    ...
    @Autowired
    public ElevatorMaintenanceServiceImpl(final ElevatorMaintenanceRepository elevatorMaintenanceRepository,
    ...
    public CalculatedCostResponse getUserCostsByUserIdAndMonth(final Integer userId, final LocalDate dateTime) {
        ...
        BigDecimal totalSpace = elevatorMaintenanceRepository.getTotalSpaceByMonthAndUserId(userId,
    ...
    private CalculatedCostResponse createElectricityObject(final BigDecimal totalSpace,
        ...
        calculatedCostResponse.setTariff("1 person - " + (Objects.nonNull(pricePerKilowatt)
                ? pricePerKilowatt : DEFAULT_PRICE_PER_RESIDENT) + "p.");
    ...
    private BigDecimal calculateTotalPrice(final BigDecimal totalSpace, final LocalDate date) {
        ...
        BigDecimal calculatedPrice = totalSpace.multiply(Objects.nonNull(pricePerKilowatt)
                ? pricePerKilowatt : DEFAULT_PRICE_PER_RESIDENT);
    ...
    private List<LocalDate> createListOfAvailableDates(final Integer userId) {
        LocalDate minMonth = getStartOfTheMonth(elevatorMaintenanceRepository.getMinMonth(userId));
        LocalDate maxMonth = getStartOfTheMonth(elevatorMaintenanceRepository.getMaxMonth(userId));
