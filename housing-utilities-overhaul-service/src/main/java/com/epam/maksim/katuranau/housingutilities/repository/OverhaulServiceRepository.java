...
@Repository
@Transactional
public interface OverhaulServiceRepository extends CrudRepository<OverhaulServiceCost, OverhaulServiceCostId> {

    void deleteByUserIdAndDateBetween(@Param("userId") Integer userId, @Param("dateFrom") LocalDate dateFrom,
                                      @Param("dateTo") LocalDate dateTo);

    @Query("SELECT MIN(es.date) FROM OverhaulServiceCost es WHERE es.userId = :userId")
    LocalDate getMinMonth(@Param("userId") Integer userId);

    @Query("SELECT MAX(es.date) FROM OverhaulServiceCost es WHERE es.userId = :userId")
    LocalDate getMaxMonth(@Param("userId") Integer userId);

    @Query("SELECT es.amountOfResourcesSpent FROM OverhaulServiceCost es WHERE es.userId = :userId AND es.date >= :dateFrom AND es.date < :dateTo")
